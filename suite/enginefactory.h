...
{
public:
  LMCMAFactory(std::string muType, std::string rng, size_t memory = 0, size_t interval = 0) : 
      EngineFactory(), _muType(muType), _rng(rng), _memory(memory), _interval(interval) {};
  ~LMCMAFactory() {};
  korali::Engine createEngine(size_t dim, double lowerbound, double upperbound, size_t maxEval, double targetFitness );
